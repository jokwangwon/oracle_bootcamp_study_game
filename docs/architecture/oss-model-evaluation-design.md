...
| **MT3** | 화이트리스트 통과율 | 모든 keyword/term이 weekly_scope 화이트리스트에 존재 (sql/description 본문 + top-level answer 배열 대상. `oracle-tokens.extractOracleTokens`가 문자열 리터럴 `'...'`/`"..."` 내부는 제외) | `apps/api/src/modules/content/services/scope-validator.service.ts` | C3 ≥ 90% |
| **MT4** | 빈칸-정답 일관성 | `template`의 `___` 개수가 `blanks` 배열 길이와 일치 + top-level `answer` 길이도 일치 + `blanks[i].answer`가 문자열 (**화이트리스트 검증은 MT3 단일 책임 — ADR-010**) | `packages/shared/src/utils/oracle-tokens.ts` (N-01 export 후 재사용) | C4 ≥ 95% |
...
| ~~**11** (v2)~~ | ~~**promptfoo 버전 호환성** — Ollama provider + custom JS assertion 2026 안정 버전~~ → **v2.4 부분 진행 (2026-04-09)**: 단계 5에서 promptfoo CLI 자체는 미설치(단계 8 R0 직전 설치 예정 — Anthropic 크레딧 충전과 동시), 그러나 본 SDD에서 요구하는 7개 assertion + langfuse-wrapped provider + dataset adapter + promptfoo.config.yaml은 모두 작성/검증 완료. 모든 코드는 promptfoo 패키지에 직접 의존하지 않고 vitest에서 단위 검증 가능 → **promptfoo 버전 변경 위험을 본 코드 경계 안에 격리**. 단계 8 직전 npm install 시점에 호환성 실측 1회 필요. | ~~Claude (npm)~~ + 사용자 (단계 8) | ~~단계 5~~ |
