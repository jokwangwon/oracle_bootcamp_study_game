...
4. **CTA 한 쌍**
   ...
   - Primary: `추천으로 시작` + `<ArrowRight />` — `bg-brand-gradient text-brand-fg`
     - 클릭 시: `오늘의 추천 주차` 로 폼 자동 채움 + **즉시 게임 시작** (`startGame({ week: currentDay })` 동시 호출). 사용자 1클릭으로 추천 세션 시작. *(Session 12 사용자 결정 2026-04-28: 명시적 confirm 패턴은 redundant 한 1클릭 추가 — 즉시 시작으로 회귀)*
   - Secondary: `이어서 학습` (마지막 학습 세션 정보 — 1초 hover 시 tooltip `Day 5 PL/SQL CURSOR · 4일 전`) — `<Button variant="outline">`
     - 클릭 시: 마지막 세션의 `topic`/`day` 로 폼 채움 + **즉시 게임 시작** (`startGame({ topic, week })`)
...
- Hero CTA `<button>` 기본
- 추천으로 시작 / 이어서 학습 → 즉시 `startGame()` 호출 (Session 12 변경 — 명시적 confirm 패턴 제거)
...
- `추천으로 시작` / `이어서 학습` 클릭 시 폼 자동 채움 + 즉시 `startGame()` 호출 (Session 12 결정)
...
| 2026-04-28 | §3.1.4 / §7.3 / §13.1 — `추천으로 시작` / `이어서 학습` 클릭 시 **즉시 게임 시작** 으로 회귀 (명시적 confirm 패턴 제거). 1클릭 추가가 redundant 라는 사용자 피드백. `startGame(overrides?: Partial<SoloConfigSelection>)` 시그니처 확장. | Session 12 사용자 결정 |
