...
| # | 게이트 | 상태 |
|---|--------|------|
| 1 | R2 전 게이트 PASS + Apache 2.0 라이센스 | ✅ 완료 (2026-04-15) |
| 2 | Ollama `qwen3-coder-next` sha256 digest 조회 + pin 자동화 | ✅ 완료 (2026-04-16) — `apps/api/src/modules/ai/eval/pins/` + `pin-model.ts` CLI + `ModelDigestProvider` 부팅 fail-closed |
| 3 | 운영 초기 100건 실사용 모니터링 파이프라인 구축 (MT3/MT4 샘플 재측정) | ✅ Phase A 완료 (2026-04-16) — `OpsModule`(measurement + report). Phase B (집계 cron) 대기 |
| 4 | 메타 편향 주의문 SDD/운영 문서 명시 | ✅ 완료 (2026-04-16) — SDD v2.8 §1.5 |
