...
#### E. CSRF 정책 — Origin/Referer 검증 (Q-R2 결정)

double-submit token **폐기**. `apps/web/src/middleware.ts` 또는 NestJS 글로벌 가드에서:
...
export class OriginGuard implements CanActivate {
  canActivate(ctx: ExecutionContext): boolean {
    ...
    if (!origin) throw new ForbiddenException('origin_missing');

    const allowed = (process.env.CORS_ORIGIN ?? '').split(',').map((s) => s.trim());
    const ok = allowed.some((a) => origin.startsWith(a));
    if (!ok) throw new ForbiddenException('origin_mismatch');
...
**근거**: 부트캠프 환경 (20명 신뢰, tailscale, 외부 3rd party 0) 에서 CSRF 위협 모델 빈약 (C). SameSite=Lax + Origin 검증 = 99% 커버 (B 도 동의). CSRF cookie httpOnly=false 로 인한 XSS 시 노출 위험 (A G9) 회피.

**유보**: PR-10c 머지 전 reviewer 재합의 — 위협 모델 변화 시 double-submit token 추가 검토.
...
PR-10a 머지 전 다음 모두 완료:

- [ ] **Tailscale spike** (~30분, 사용자 환경) — Lax + Origin + secure 분기 + http://api.local 검증 (A CRITICAL-3)
- [ ] **ADR-007 변경 영향 분석** — JwtAuthGuard 9 컨트롤러 + web 6 페이지 호출처 추적 (A G8)
- [ ] **refresh_tokens migration TDD** — 11+ cases (A·B 합의, B의 60+ tests 추정의 일부)
- [ ] **token_epoch ALTER TABLE migration TDD**
- [ ] **OriginGuard 글로벌 등록 + CSRF token 폐기 e2e**
- [ ] **임시 완화 24h → 정식 30m 복귀** (.env / .env.example / env.validation.ts default)

#### J. 합의 메타

| 항목 | 출처 |
|------|------|
| 합의 보고서 | `docs/review/consensus-010-pr-10-security.md` (Session 12 작성) |
...
| 합의율 | 70% 만장일치 7 / 부분 4 / 불일치 3 / 누락 채택 5 |
